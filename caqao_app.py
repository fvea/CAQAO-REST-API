...
model.cuda()  # GPU
cacao_image_classifier = tf.keras.models.load_model('mobnet_0003.h5')
...
def modify_boxes_color(results, image):
   # Convert the results to a pandas dataframe and extract the first row
    ...
    for i, row in df.iterrows():
        ...
        try:
            ...
            cv2.rectangle(image, (x1, y1), (x2, y2), colors[class_name], 10)
        except KeyError:
            cv2.rectangle(image, (x1, y1), (x2, y2), (0, 0, 255), 10)

    return image

def modify_boxes_fissuring(results, image):
    # Convert the results to a pandas dataframe and extract the first row
    df = results.pandas().xyxy[0]
    for i, row in df.iterrows():
        # Extract the class index and bounding box coordinates
        # class_idx = int(row['class'])
        class_name = row['name'].split("-")[-1].strip() if row['name'] != 'Insect-damaged' else 'Insect-damaged'
        x1, y1, x2, y2 = int(row['xmin']), int(row['ymin']), int(row['xmax']), int(row['ymax'])
        # Draw the bounding box on the image using the class's color
        # color = tuple(colors[class_idx])
        try:
            font_scale = 0.28 * (y2 - y1) / 30
            cv2.putText(image, class_name, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, font_scale, fissuring[class_name], 3)
            cv2.rectangle(image, (x1, y1), (x2, y2), fissuring[class_name], 20)
        except KeyError:
...
def assess():
    ...
    if request.method == "POST":
        ...
        image_detection = modify_boxes_color(results, np.asarray(image))
